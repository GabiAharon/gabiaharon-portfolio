...
export default function Projects() {
  ...
  // Subtle floating particles - much lighter than before
  const [subtleParticles, setSubtleParticles] = useState([]);

  useEffect(() => {
    // Create subtle floating particles - much lighter animation
    const particleCount = window.innerWidth < 768 ? 5 : 10; // Even fewer for projects page
    const initialParticles = Array.from({ length: particleCount }, (_, i) => ({
      id: i,
      x: Math.random() * 100,
      y: Math.random() * 100,
      size: Math.random() * 1.2 + 0.4, // Smaller particles
      speed: Math.random() * 0.015 + 0.008, // Very slow
      opacity: Math.random() * 0.25 + 0.08, // Very subtle
    }));
    setSubtleParticles(initialParticles);

    // Very light animation
    const interval = setInterval(() => {
      setSubtleParticles(prev => prev.map(p => ({
        ...p,
        y: (p.y + p.speed) % 100,
        opacity: 0.08 + Math.abs(Math.sin(Date.now() * 0.0003 + p.id)) * 0.15,
      })));
    }, 600); // Even slower for projects page
...
או רק טוקן (אם רוצה להשתמש ב-${defaultOwner}/${defaultRepo})`);
  ...
  return (
    <div className={`min-h-screen w-full bg-gradient-to-b from-slate-950 via-gray-950 to-black text-white relative overflow-hidden ${language === 'he' ? 'rtl' : 'ltr'}`}>
      {/* Enhanced gradient background with subtle particles */}
        ...
        <div className="relative z-10 max-w-6xl mx-auto px-6 py-8">
          ...
          >
            ...
            <div className="flex items-center gap-4 mb-4">
              <h1 className="text-4xl md:text-5xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-slate-100 to-slate-300">
                {t('projectsPageTitle')}
              </h1>
              {isEditMode && (
              ...
              {categories.map((category) => (
                <motion.button
                  ...
                  className={`flex items-center gap-2 px-4 py-2 rounded-full relative overflow-hidden ${
                    selectedCategory === category.id
                      ? 'bg-slate-700 text-white shadow-lg shadow-slate-600/30'
                      : 'bg-slate-950 text-slate-400'
                  }`}
                  whileHover={{ 
                    scale: selectedCategory === category.id ? 1 : 1.05,
                    transition: { duration: 0.2, ease: "easeOut" }
                  }}
                  whileTap={{ 
                    scale: 0.95,
                  ...
                  animate={{
                    backgroundColor: selectedCategory === category.id ? '#1e293b' : '#0f172a',
                ...
                >
                  ...
                  <motion.div
                    ...
                    whileHover={{ 
                      opacity: selectedCategory === category.id ? 0 : 0.3,
                      transition: { duration: 0.2 }
                    }}
                  />
            ...
            <AnimatePresence>
              {filteredProjects.map((project, index) => (
                <motion.div
                  ...
                  whileHover={{ 
                    scale: editingProject === project.id ? 1 : 1.03,
                    y: editingProject === project.id ? 0 : -5,
                    transition: { 
                      duration: 0.3, 
                      ease: "easeOut",
                      type: "spring",
                  ...
                  whileTap={{ 
                    scale: editingProject === project.id ? 1 : 0.97,
                    transition: { duration: 0.15, ease: "easeInOut" }
                  }}
                >
                  <div className={`${editingProject === project.id && !editingInModal ? 'bg-gradient-to-r from-slate-600 to-slate-700' : 'bg-gradient-to-r from-slate-700 to-blue-900'} p-0.5 rounded-xl w-full flex`}>
                    <motion.div
                      className="bg-gray-900 rounded-xl overflow-hidden flex flex-col w-full min-h-[600px] relative"
                      style={{ 
                      ...
                      onMouseMove={(e) => {
                        if (editingProject === project.id || !e.currentTarget) return;
                    ...
                    >
                      ...
                      <div className="relative h-48 overflow-hidden flex-shrink-0">
                        <motion.img 
                          ...
                          whileHover={{
                            scale: editingProject === project.id ? 1 : 1.08,
                            transition: {
                              duration: 0.6,
                              ...
                              <div className="mt-auto">
                                {project.isVideo && project.link && !project.link.includes('#') ? (
                                  ...
                                  // כפתור לצפייה מהירה
                                    <motion.button
                                      onClick={() => setSelectedProject(project)}
                                      className="flex items-center justify-center gap-2 w-full bg-purple-600 text-white py-2 px-4 rounded-lg text-sm relative overflow-hidden"
                                      whileHover={{ 
                                    ...
                                    >
                                      <motion.div
                                        className="absolute inset-0 bg-gradient-to-r from-slate-600 to-slate-700 opacity-0"
                                        whileHover={{ opacity: 1 }}
                                        transition={{ duration: 0.3 }}
                                      />
                                      <ExternalLink className="w-4 h-4 relative z-10" />
                                      <span className="relative z-10">{t('quickView')}</span>
                                    </motion.button>
                                )}
                              </div>
                            </div>
