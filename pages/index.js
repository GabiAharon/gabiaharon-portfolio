...
const FloatingParticles = () => {
  const isMobile = useIsMobile();
  const particleCount = isMobile ? 10 : 30; // Less particles on mobile
...
const CinematicIntro = ({ onComplete, language }) => {
  ...
  useEffect(() => {
    const timers = [
      setTimeout(() => setPhase(1), 300),    // Start spotlight
      setTimeout(() => setPhase(2), 1000),   // Show name
      setTimeout(() => setPhase(3), 2200),   // Show title
      setTimeout(() => setPhase(4), 3500),   // Start exit
  ...
  return (
    ...
    >
      ...
      {/* Central Spotlight - Smaller on mobile */}
      <motion.div
        className="absolute"
      ...
      {/* Light Rays - Fewer and shorter on mobile */}
    ...
    <>
      ...
      <div className={`min-h-screen bg-gradient-to-b from-slate-950 via-gray-950 to-black text-white overflow-x-hidden ${isHebrew ? 'rtl' : 'ltr'}`} style={{ fontFamily: "'Heebo', 'Assistant', sans-serif" }}>
        ...
        <section id="about" className="py-24 relative">
          ...
          {/* Decorative floating shapes - Hidden on mobile to prevent overflow */}
          <motion.div
        ...
        <section id="contact" className="py-24 relative">
          ...
          {/* Animated gradient blobs - Smaller on mobile, hidden movement */}
          <motion.div
